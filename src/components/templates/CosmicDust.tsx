...
import s from '@/styles/cosmic.module.css';
...
export default function CosmicDust() {
  ...
  return (
    <div className={s.template}>
      {/* Starfield & Nebula layers */}
      <div className={s.stars} />
      <div className={s.nebula} />

      {/* Hero — ethereal centered */}
          ...
          <motion.div variants={photoReveal}>
            <div className={s.photoOuter}>
              ...
              <div className={s.photoClip}>
                <Image src={photoUrl} alt="Cristian Torres" fill sizes="170px" className="profile-photo" priority />
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={heroItem} className={s.stats}>
            {data.stats.map((stat, i) => (
              <motion.div
                ...
                transition={{ delay: 1.3 + i * 0.15, duration: 0.6, type: 'spring' as const, stiffness: 150 }}
              >
      ...
      {/* Skills */}
      <div className={s.sec}>
        <SectionHeader tag="Constellation" title="Technical Skills" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <StaggerChildren className={s.sg} stagger={0.1}>
          {data.skills.map((skill, i) => (
            <motion.div key={i} variants={staggerItem} whileHover={floatHover} className={s.sc}>
      ...
      <div className={s.sec} data-code-showcase>
        <SectionHeader tag="Live Signal" title="Inside My Editor" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <CodeShowcase />
      </div>

      {/* Experience */}
      <div className={s.sec}>
        <SectionHeader tag="Orbit" title="Experience" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <div className={s.tl}>
          {data.exp.map((exp, i) => (
      ...
      {data.projects.length > 0 && (
        <div className={s.sec}>
          <SectionHeader tag="Nebula" title="Projects" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
      ...
      <div id="contact" className={s.sec}>
        <SectionHeader tag="Transmission" title="Get In Touch" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
