...
import s from '@/styles/brutal.module.css';
...
export default function Brutalist() {
  ...
  return (
    <div className={s.template}>
      {/* Hero — raw bordered layout */}
      <div className={s.hero}>
        <motion.div className={s.heroInner} initial="hidden" animate="visible" variants={heroContainer}>
          <div>
          ...
          <motion.div variants={photoReveal}>
            <div className={s.photoWrap}>
              <Image src={photoUrl} alt="Cristian Torres" fill sizes="240px" className="profile-photo" priority style={{ objectFit: 'cover' }} />
      ...
      {/* Skills — brutal grid */}
      <div className={s.sec}>
        <SectionHeader tag="Stack" title="Skills" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <StaggerChildren className={s.sg} stagger={0.06}>
          {data.skills.map((skill, i) => (
            <motion.div key={i} variants={staggerItem} whileHover={brutalHover} className={s.sc}>
      ...
      <div className={s.sec} data-code-showcase>
        <SectionHeader tag="Live" title="Editor" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <CodeShowcase />
      </div>

      {/* Experience */}
      <div className={s.sec}>
        <SectionHeader tag="Work" title="Experience" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
        <div className={s.tl}>
          {data.exp.map((exp, i) => (
      ...
      {data.projects.length > 0 && (
        <div className={s.sec}>
          <SectionHeader tag="Output" title="Projects" tagClass={s.secTag} titleClass={s.secTitle} wrapperClass={s.secH} />
          <StaggerChildren className={s.pg}>
            {data.projects.map((project, i) => (
              <motion.a
                key={i}
                variants={staggerItem}
                whileHover={{ x: -4, y: -4, transition: { type: 'spring' as const, stiffness: 600, damping: 30 } }}
                href={project.url}
                target="_blank"
